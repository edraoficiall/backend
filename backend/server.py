...
async def export_pdf(
    ...
    # Table data
    data = [["Fecha", "Tipo", "Descripción", "Monto (CUP)"]]
    for t in transactions:
        tipo = "Ingreso" if t["type"] == "income" else "Gasto"
        monto = f"{t['amount']:,.2f}" if t["type"] == "income" else f"-{t['amount']:,.2f}"
        data.append([t["date"], tipo, t["description"][:30], monto])
    
    if len(data) > 1:
        table = Table(data, colWidths=[80, 60, 200, 100])
    ...
    elements.append(Paragraph(f"<b>Total Ingresos:</b> {total_income:,.2f} CUP", styles['Normal']))
    elements.append(Paragraph(f"<b>Total Gastos:</b> {total_expenses:,.2f} CUP", styles['Normal']))
    elements.append(Paragraph(f"<b>Balance:</b> {balance:,.2f} CUP", styles['Heading2']))
